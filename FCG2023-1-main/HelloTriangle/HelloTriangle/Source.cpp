...
//#endif
	...
	//Exemplo da criação de círculo
	int nPoints = 200;
	GLuint VAO = createCircle(0.5, nPoints);

	//Exemplo de criação pacman/fatia de pizza
	int nPoints2;
	GLuint VAO2 = createPie(0.5, 45.0, 325.0, nPoints2);

	int nPoints3;
	GLuint VAO3 = createSpiral(5, nPoints3);
...
int createCircle(float radius, int nPoints)
{
	...
	float angle = 0.0;
	float slice = 2 * Pi / ((float)nPoints - 2.0);
...
int createSpiral(int turns, int& nPoints)
{
	...
	float g = rand() % 11 / 10.0;
	float b = rand() % 11 / 10.0;
